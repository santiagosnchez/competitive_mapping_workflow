...
autoXtype.male = df.AS %>% filter(sex == "male") %>% dplyr::select(genes, chromosome, type)
autoXtype.male$chromosome2 = "autosomes"
autoXtype.male$type = as.character(autoXtype.male$type)

autoXtype.male$type[ autoXtype.male$type == "ambiguous" ] = "other"
autoXtype.male$type[ autoXtype.male$type == "cis x trans (compensatory)" ] = "other"
autoXtype.male$type[ autoXtype.male$type == "cis + trans (enhancing)" ] = "other"

tmp2 = df.inherit2 %>% filter(sex == "male" & chromosome == "X") %>% dplyr::select(gene, chromosome, class, bn.bf.nf) %>% rename(genes = "gene", type = "class")
tmp2$chromosome2 = "X"
tmp2$type = as.character(tmp2$type)
tmp2$type[ tmp2$type == "ambiguous" ] = "other"
tmp2$type[ tmp2$type == "additive" ] = "other"
tmp2$type[ tmp2$type == "no change" ] = "conserved"
tmp2$type[ tmp2$type == "C. nigoni dominant" ] = "cis-only"
tmp2$type[ tmp2$type == "C. briggsae dominant" ] = "trans-only" # with trans-dominant effect
tmp2$type[ tmp2$bn.bf.nf == "0 -1 -1" ] = "cis-trans (compensatory)" # subgroup of underdominant genes
tmp2$type[ tmp2$bn.bf.nf == "0 1 1" ] = "cis-trans (compensatory)" # subgroup of overdominant genes
tmp2$type[ tmp2$type == "overdominant" ] = "other"
tmp2$type[ tmp2$type == "underdominant" ] = "other"

autoXtype.male = rbind(autoXtype.male, tmp2 %>% dplyr::select(-bn.bf.nf))

autoXtype.male.count = autoXtype.male %>% group_by(chromosome2, type) %>% count()
autoXtype.male.count$enrichment = as.numeric(enrichment(matrix(autoXtype.male.count$n, nrow=5), odds.ratio=T))
autoXtype.male.count$p.value = as.numeric(enrichment(matrix(autoXtype.male.count$n, nrow=5), odds.ratio=F))
#autoXtype.male.count$enrichment = as.numeric(t(enrichment(matrix(autoXtype.male.count$n, ncol=5, nrow=6, byrow=T), odds.ratio=T)))
#autoXtype.male.count$p.value = as.numeric(t(enrichment(matrix(autoXtype.male.count$n, ncol=5, nrow=6, byrow=T), odds.ratio=F)))

autoXtype.male.count$sex = "male"

# females

autoXtype.female = df.AS %>% filter(sex == "female") %>% dplyr::select(genes, chromosome, type)
autoXtype.female$chromosome2 = "autosomes"
autoXtype.female$chromosome2[ autoXtype.female$chromosome == "X" ] = "X"
autoXtype.female$type = as.character(autoXtype.female$type)

autoXtype.female$type[ autoXtype.female$type == "ambiguous" ] = "other"
autoXtype.female$type[ autoXtype.female$type == "cis x trans (compensatory)" ] = "other"
autoXtype.female$type[ autoXtype.female$type == "cis + trans (enhancing)" ] = "other"

autoXtype.female.count = autoXtype.female %>% group_by(chromosome2, type) %>% count()
autoXtype.female.count$enrichment = as.numeric(enrichment(matrix(autoXtype.female.count$n, nrow=5), odds.ratio=T))
autoXtype.female.count$p.value = as.numeric(enrichment(matrix(autoXtype.female.count$n, nrow=5), odds.ratio=F))
# autoXtype.female.count$enrichment = as.numeric(t(enrichment(matrix(autoXtype.female.count$n, ncol=5, nrow=6, byrow=T), odds.ratio=T)))
# autoXtype.female.count$p.value = as.numeric(t(enrichment(matrix(autoXtype.female.count$n, ncol=5, nrow=6, byrow=T), odds.ratio=F)))

autoXtype.female.count$sex = "female"

autoXtype.enrich = rbind(autoXtype.female.count, autoXtype.male.count)
#autoXtype.enrich = autoXtype %>% filter(enrichment >= 1)

autoXtype.enrich$type = as.character(autoXtype.enrich$type)
autoXtype.enrich$type[ autoXtype.enrich$type == "trans-only" ] = "trans-only*"
autoXtype.enrich$type = factor(autoXtype.enrich$type, c("conserved","other","trans-only*","cis-only","cis-trans (compensatory)"))
levels.legend = c("conserved","other",expression(paste(italic("trans"),"-only*",sep="")),expression(paste(italic("cis"),"-only", sep="")),expression(paste(italic("cis-trans"), " (compensatory)")))

autoXtype.enrich$chromosome3 = c(1,2)[ factor(autoXtype.enrich$chromosome2) ]

autoXtype.enrich$sig = ""
autoXtype.enrich$sig[ autoXtype.enrich$p.value < 0.05 & abs(log2(autoXtype.enrich$enrichment)) > 0.5 ] = "*"

# rectX = data.frame(chromosome3=2, x1=1.5, x2=2.5, y1=0, y2=2.2)

# ggplot(autoXtype.enrich, aes(x=chromosome3, y=log2(enrichment))) +
#     geom_rect(data=rectX, aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill="grey90", alpha=0.5, inherit.aes=F) +
#     geom_bar(aes(fill=type), stat="identity", position=position_dodge(width=0.9), alpha=0.7) +
#     facet_rep_wrap(~sex, ncol=1, strip.position="right") +
#     scale_fill_manual(values=cols.type[c(1:4,7)], name="", labels=levels.legend, drop=FALSE) +
#     scale_x_continuous(breaks=c(1,2), labels=c("autosomes","X")) +
#     scale_y_continuous(expand=c(0,0)) +
#     labs(x="", y=expression(paste("enrichment (log"[2], " odds ratio)"))) +
#     background_grid(major="y", size.major = 0.2, colour.major = "grey75") +
#     #geom_text(aes(y=log2(enrichment)/2, label=sig), position=position_dodge(width=0.9), size=5, show.legend=F, color="white") +
#     theme(legend.text=element_text(hjust=0),
#         strip.background = element_rect(fill="grey90", color=NA))

Xenrich = ggplot(autoXtype.enrich %>% filter(chromosome2 == "X"), aes(x=log2(enrichment), y=type)) +
    #geom_rect(data=rectX, aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill="grey90", alpha=0.5, inherit.aes=F) +
    geom_vline(xintercept=0, size=1) +
    geom_bar(aes(fill=type), stat="identity", position=position_dodge(width=0.9), alpha=0.7, show.legend=F) +
    facet_rep_wrap(~sex, ncol=2, strip.position="top") +
    scale_fill_manual(values=cols.type[c(1:4,7)], name="", labels=levels.legend, drop=FALSE) +
    scale_x_continuous(position="bottom", limits=c(-1,1), labels=c("1","","0","","1")) +
    annotate(geom="label", label="autosomes", y="trans-only*", x=-.75, fill="black", color="white") +
    annotate(geom="label", label="X", y="trans-only*", x=.75, fill="black", color="white") +
    scale_y_discrete(labels=levels.legend) +
    #scale_y_continuous(expand=c(0,0)) +
    labs(y="", x=expression(paste("enrichment (log"[2], " odds ratio)"))) +
    background_grid(major="x", size.major = 0.2, colour.major = "grey75") +
    geom_text(aes(x=log2(enrichment)/2, label=sig), size=5, show.legend=F, color="white") +
    theme(legend.text=element_text(hjust=0),
        strip.background = element_rect(fill="grey90", color=NA))

Xcount = ggplot(autoXtype.enrich, aes(x=chromosome2, y=n, fill=type)) +
    geom_bar(stat="identity", position=position_dodge(), show.legend=F, alpha=0.7) +
    scale_y_continuous(breaks=seq(0,4000,500), labels=c("0","","1000","","2000","","3000","","4000")) +
    # scale_y_log10() +
    # annotation_logticks(sides="l") +
    facet_rep_wrap(~sex) +
    scale_fill_manual(values=cols.type[c(1:4,7)], name="", labels=levels.legend, drop=FALSE) +
    labs(x="", y=expression(paste("number of genes (log"[10], ")"))) +
    background_grid(major="y", size.major = 0.2, colour.major = "grey75") +
    theme(legend.text=element_text(hjust=0),
        strip.background = element_rect(fill="grey90", color=NA))

plot_grid(Xenrich, Xcount, nrow=2, align="v")

ggsave("figures/Fig3_X_vs_auto_regulatory_div.pdf", device="pdf", useDingbats=F)
ggsave("figures/Fig3_X_vs_auto_regulatory_div.png")
