...
pE = ggplot(df.AS, aes(logFC.sp, logFC.ase, color=type)) +
...
pF = ggplot(df.AS, aes(y=type, x=abs(logFC.sp), fill=type)) +
	geom_boxploth(alpha=0.7, outlier.shape=NA, width=0.5, show.legend=F) +
	geom_density_ridges(alpha=0.7, color=NA, show.legend=F) +
	scale_y_discrete(limits=rev(type.levels)) +
	scale_fill_manual(values=cols.type) +
	facet_rep_wrap(~sex, ncol=1, strip.position="right") +
	xlab("|log2-FC expression divergence|") +
	ylab("") +
	background_grid(major="x", minor="x", size.major = 0.2, colour.major = "grey75", size.minor = 0.2, colour.minor = "grey75") +
	xlim(0,10) +
	theme(strip.background = element_blank(), strip.text=element_blank(),
...
row2 = plot_grid(pE + ggtitle("A") + theme(plot.margin = unit(c(0, 0, 0, 0), "cm")),
								pF + ggtitle("B") + theme(plot.margin = unit(c(0, 0.5, 0, 0), "cm")),
								pG + ggtitle("C") + theme(plot.margin = unit(c(0, 0.5, 0, 0), "cm")),
								pH + ggtitle("D") + theme(plot.margin = unit(c(0, 0.5, 0, 0), "cm")),
...
f4_1 = ggplot(class.vs.type %>% filter(sex == "female" | sex == "male"), aes(x=class, y=type, fill=n)) +
    ...
    scale_y_discrete(labels=type.legend[3:7]) +
    scale_x_discrete(labels=class.legend[3:7]) +
    theme(strip.background = element_rect(fill="grey90", color=NA),
