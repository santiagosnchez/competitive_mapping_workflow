...
cols.inherit = rev(c("#A68481","#FCA79F","#6F7862","#B379AF","#4C5E91"))
...
df.dds.rlog.females.m = pivot_longer(dds.rlog.females.m, c(-chromosome, -rel.pos, -pseudo.pos), names_to="species", values_to="log_norm_expr")
...
mds.auto.plt = ggplot(df.mds, aes(x=dim1, y=dim2, color=sex, shape=species)) +
	...
	scale_color_manual(values=c(cols.sex[1],cols.inherit[5],cols.sex[2]), labels=c("female","hermaphrodite","male")) +
...
enrich.div.chr.plt  = ggplot(df.div.chr.enrich, aes(x=chr.pseudo, y=log2(enrichment), fill=species_DE)) +
...
sex.de.plt = ggplot(df.DE.sex.counts, aes(x=species, y=count, color=sex, group=sex)) +
	...
	scale_color_manual(values=c(cols.sex[1],cols.inherit[5],cols.sex[2], cols.sex[3]), name="", labels=c("female-biased","hermaphrodite","male-biased","sex-neutral")) +
...
enrich.sex.plt  = ggplot(df.DE.sex.enrich, aes(x=chr.pseudo, y=log2(enrichment), fill=sex)) +
    ...
    scale_fill_manual(values=c(cols.sex[1],cols.inherit[5],cols.sex[2], cols.sex[3]), name="", labels=c("female","hermaphrodite","male","sex-neutral")) +
