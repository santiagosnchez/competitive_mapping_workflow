...
exprdiff = rbind(cbind(expr.div, sp.vs.allele="expression divergence"), cbind(ase.div, sp.vs.allele="ASE"))
exprdiff$species_DE = as.character(exprdiff$species_DE)
exprdiff$species_DE[ exprdiff$DE == "no sig" ] = "no sig"

exprdiff.plt = ggplot(exprdiff, aes(logFC, fill=species_DE)) +
...
enrich.div.chr.plt  = ggplot(df.div.chr.enrich, aes(x=chr.pseudo, y=log2(enrichment), fill=species_DE)) +
	...
	scale_fill_manual(values=c(cols.spp[c(1,2)],"grey"), name="", labels=c(
		...
		expression(italic("C. nigoni")),
		"no DE")) +
    scale_y_continuous(breaks=seq(-2,2,0.5), limits=c(-2,2,1), expand=c(0,0)) +
	...
	theme(strip.background = element_rect(fill="grey90", color=NA),
				...
				legend.text.align = 0, legend.position=c(0.82,0.15),
...
sex.de.plt = ggplot(df.DE.sex.counts, aes(x=species, y=count, color=sex, group=sex)) +
	...
	scale_y_continuous(limits=c(1000,7000), expand=c(0,0),
...
enrich.sex.plt  = ggplot(df.DE.sex.enrich, aes(x=chr.pseudo, y=log2(enrichment), fill=sex)) +
    ...
    scale_fill_manual(values=cols.sex, name="", labels=c("female^","male","sex-neutral")) +
    scale_y_continuous(breaks=seq(-2,2,0.5), limits=c(-2,2,1), expand=c(0,0)) +
...
# supplementary
ggsave("figures/suppl_FigS1_expr_MDS.pdf", device="pdf", useDingbats=FALSE)
ggsave("figures/suppl_FigS1_expr_MDS.png", device="png")
