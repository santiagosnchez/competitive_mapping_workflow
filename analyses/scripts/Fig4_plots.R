...
heatmap = ggplot(df.models2, aes(x=key, y=models, fill=value)) +
	...
	scale_y_discrete(limits=rev(models.levels), labels=table(df.species_sex_class$models)[ rev(models.levels) ]) +
...
expr_div = ggplot(df.species_sex_class, aes(y=models, x=abs(logFC_species))) +
	geom_density_ridges(aes(fill=sex), color=NA, alpha=0.5, show.legend=F) +
	...
	scale_y_discrete(limits=rev(models.levels), labels=paste("M",12:0,sep="")) +
...
aa_div = ggplot(df.species_sex_class, aes(y=models, x=Ka/Ks_ENCc)) +
	geom_density_ridges(aes(fill=sex), color=NA, alpha=0.5, show.legend=F) +
	geom_boxploth(alpha=0.5, outlier.shape=NA, width=0.5) +
	scale_fill_manual(values=cols.sex) +
...
prop_class = ggplot(df.species_sex_class.expr_class.counts, aes(x=models, y=n, fill=class)) +
...
prop_type = ggplot(df.species_sex_class.expr_type.counts, aes(x=models, y=n, fill=type)) +
	geom_bar(stat="identity", position="fill", show.legend=F, alpha=0.7) +
	scale_y_continuous(position="right", labels=c("0","","0.5","","1")) +
	scale_x_discrete(limits=rev(models.levels)) +
...
row1 = plot_grid(heatmap + theme(plot.margin = unit(c(t=-0.3, r=-0.1, b=0, l=-0.2), "cm")),
					...
					rel_widths=c(0.1,0.2,0.2,0.4,0.4), nrow=1)
